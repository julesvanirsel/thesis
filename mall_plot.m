direc = '\\Dartfs-hpc\rc\lab\L\LynchK\public_html\Gemini3D\aurora_E2BG_05\';
alt_ref = 170;
lon_ref = 600;
fs = 20;
xlims = [-1,1]*880;
...
%     E0 = E0gi(MLAT,MLON)/1e3;
    ...
    p0 = [[750,40,alt_ref];[500,150,alt_ref];[xlims(2),100,115]];
    r0 = [100,100,10];
    r1 = [20,20,45];
    v0 = [[1,0,0];[1,0,0];[0,0,1]];
    rev = [1,1,1];
    res = [32,32,16];
    colors = [[0,0.7,0];[0.7,0,1];[0,0,0]];
    ntubes = size(p0,1);
    for n = 1:ntubes
        tubes.(char(64+n)) = fluxtube(xg,dat,alt_ref,p0(n,:),r0(n),r1(n),v0=v0(n,:)...
...
%     clb.Position = [0.86,0.15,0.017,0.36];
    ...
    clb.FontSize = fs;
    clb.Position = [0.86,0.55,0.017,0.36];
    clb.Ticks = [10,11];

    annotation('textarrow',[0.55,0.59],[0.878,0.85] ...
        ,'String','10 mV/m ','HeadLength',10,'HeadWidth',10,'LineWidth',2,'Color','k','FontSize',fs)
...
%         set(shd,'FaceAlpha',0.5)
        ...
        set(stl,'Color',color,'LineWidth',1.5)
        end
    end
    ...
    fn = 'D:\Files\research\FINESST\mall.png';
    saveas(gcf,fn)
    disp(fn)
    close all
end
