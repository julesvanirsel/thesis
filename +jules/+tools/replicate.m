...
for track_id = 1:num_tracks
    ...
    if opts.show_plots
        ...
        plot(x3_traj_old*scl.x,(track.flow(:,1) - v_bg(1))*scl.v,'Color',[1,0.5,0.5])
        plot(x3_traj_old*scl.x,(track.flow(:,2) - v_bg(2))*scl.v,'Color',[0.5,0.5,1])
    ...
    v2_int = griddata(x2_traj_rep(:),x3_traj_rep(:),v2_traj_rep(:),X2,X3,'cubic'); %#ok<*GRIDD> need up to cubic
    ...
    if num_tracks > 1
        interpolations.(track_name).v2_int = v2_int;
        interpolations.(track_name).v3_int = v3_int;
    end
    x2_traj_p = [x2_traj_p;x2_traj]; %#ok<AGROW>
    x3_traj_p = [x3_traj_p;x3_traj]; %#ok<AGROW>
    v2_traj_p = [v2_traj_p;v2_traj]; %#ok<AGROW>
    v3_traj_p = [v3_traj_p;v3_traj]; %#ok<AGROW>
...
if opts.show_plots
    ...
    quiver(x2_traj_p*scl.x,x3_traj_p*scl.x, ...
        v2_traj_p*scl.v*vs,v3_traj_p*scl.v*vs,0,'.-g')
    xlim(lim.x); ylim(lim.y)
    xlabel(lbl.x); ylabel(lbl.y)
    colormap(colorcet(clm.arc))
    clb = colorbar;
