...
for i = 1:length(dxs)
    ...
    if i==50 && do_plot
        ...
        quiver(x2_traj*scl.x,x3_traj*scl.x, ...
            v2_traj*scl.vec,v3_traj*scl.vec,0,'.-g')
        ...
        quiver(x2_traj_rep(i,:)*scl.x,x3_traj_rep(i,:)*scl.x, ...
            v2_traj*scl.vec,v3_traj*scl.vec,0,'.-r')
        xlim(lim.x)
        ylim(lim.y)
        xlabel(lbl.x)
        ylabel(lbl.y)
...
if do_plot
    ...
    quiver(x2_traj*scl.x,x3_traj*scl.x,v2_traj*scl.vec,v3_traj*scl.vec,0,'.-k')
    xlim(lim.x)
    ylim(lim.y)
    xlabel(lbl.x)
    ylabel(lbl.y)
...
if auto_lim
    qnt = 0.9;
    max_ve = quantile(abs(v2_int(:)+plot_bg*v2_bg),qnt);
    max_vn = quantile(abs(v3_int(:)+plot_bg*v3_bg),qnt);
    max_dv = quantile(abs(divv_int(:)),qnt);
    max_p = quantile(abs(phitopA(:)),qnt);
    lim.ve = [-1,1]*max_ve*scl.v;
    lim.vn = [-1,1]*max_vn*scl.v;
...
hold on
pcolor(X2*scl.x,X3*scl.x,(v2_int+plot_bg*v2_bg)*scl.v)
quiver(x2_traj*scl.x,x3_traj*scl.x,v2_traj*scl.vec,v3_traj*scl.vec,0,'.-r')
quiver(48,-49,1e3*scl.vec,0,0,'.-r')
text(71,-48,'1 km/s','FontSize',0.8*fts)
...
text(0.04,0.9,char(ltr),'units','normalized','FontSize',fts*0.8); ltr = ltr+1;
hold on
pcolor(X2*scl.x,X3*scl.x,(v2_A+plot_bg*v2_bg)*scl.v)
quiver(x2_traj*scl.x,x3_traj*scl.x,v2_traj*scl.vec,v3_traj*scl.vec,0,'.-r')
colormap(gca,colorcet(clm.v))
xlim(lim.x); ylim(lim.y); clim(lim.ve)
xticks([]); yticks([])
...
text(0.04,0.9,char(ltr),'units','normalized','FontSize',fts*0.8); ltr = ltr+1;
hold on
pcolor(X2*scl.x,X3*scl.x,(v2_C+plot_bg*v2_bg)*scl.v)
quiver(x2_traj*scl.x,x3_traj*scl.x,v2_traj*scl.vec,v3_traj*scl.vec,0,'.-r')
colormap(gca,colorcet(clm.v))
clb = colorbar;
...
quiver(x2_traj*scl.x,x3_traj*scl.x,v2_traj*scl.vec,v3_traj*scl.vec,0,'.-r')
colormap(gca,colorcet(clm.v))
xlim(lim.x); ylim(lim.y); clim(lim.vn)
xticks([])
ylabel(lbl.y)
pbaspect(ar)
...
text(0.04,0.9,char(ltr),'units','normalized','FontSize',fts*0.8); ltr = ltr+1;
hold on
pcolor(X2*scl.x,X3*scl.x,(v3_A+plot_bg*v3_bg)*scl.v)
quiver(x2_traj*scl.x,x3_traj*scl.x,v2_traj*scl.vec,v3_traj*scl.vec,0,'.-r')
colormap(gca,colorcet(clm.v))
xlim(lim.x); ylim(lim.y); clim(lim.vn)
xticks([]); yticks([])
...
text(0.04,0.9,char(ltr),'units','normalized','FontSize',fts*0.8); ltr = ltr+1;
hold on
pcolor(X2*scl.x,X3*scl.x,(v3_C+plot_bg*v3_bg)*scl.v)
quiver(x2_traj*scl.x,x3_traj*scl.x,v2_traj*scl.vec,v3_traj*scl.vec,0,'.-r')
colormap(gca,colorcet(clm.v))
clb = colorbar;
...
quiver(x2_traj*scl.x,x3_traj*scl.x,v2_traj*scl.vec,v3_traj*scl.vec,0,'.-r')
colormap(gca,colorcet(clm.v))
xlim(lim.x); ylim(lim.y); clim(lim.ve/3)
xticks([]); yticks([])
pbaspect(ar)
...
% filename = fullfile('plots',filename);
filename = fullfile(pwd,'plots','paper0','helmholtz-comparison.png');
if save_plot
...
if auto_lim
    qnt = 0.95;
    max_v = quantile(abs([v2_int(:)+plot_bg*v2_bg;v3_int(:)+plot_bg*v3_bg]),qnt);
    max_dv = quantile(abs(divv_int(:)),qnt);
    max_p = quantile(abs(phitopA(:)),qnt);
...
figure
set(gcf,'PaperPosition',[0,0,13.2,4.3]) %4.3
...
filename = fullfile(pwd,'plots','paper0','harmonic.png');
if save_plot
    fprintf('Saving file: %s\n',filename)
    saveas(gcf,filename)
end
